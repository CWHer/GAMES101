...
{
    std::cout << "MSAA: " << MSAA_N << std::endl;

    const bool OUTPUT_FRAME_INFO = false;
    for (int x = 0; x < width; ++x)
        for (int y = 0; y < height; ++y)
        {
            if (OUTPUT_FRAME_INFO)
            ...
            if (OUTPUT_FRAME_INFO)
            {
                std::cout << frame_buf[idx] << std::endl;
                std::cout << std::endl;
            }
        }
}
        ...
        for (int y = min_y; y < max_y; ++y)
            for (int i = 0; i < offsets.size(); ++i)
            {
                                   ...
                                   t.v))
                {
                    ...
                    // NOTE: barycentric coordinates are not invariant under projection
                    ...
                    if (z_interpolated < sample_dep_buf[get_index(x, y)][i])
                    {
                        ...
                        // This is what cause bug !!!!
                        // auto interpolate = [alpha, beta, gamma](auto v)
                        // These are OK.
                        // auto interpolate = [alpha, beta, gamma](const auto &v)
                        auto interpolate = [alpha, beta, gamma]<class T>(const T &v)
                        {
                            return alpha * v[0] + beta * v[1] + gamma * v[2];
                        };

                        auto interpolated_color = interpolate(t.color);
                        auto interpolated_normal = interpolate(t.normal).normalized();
                        auto interpolated_texcoords = interpolate(t.tex_coords);
                        auto interpolated_shadingcoords = interpolate(view_pos);
                        ...
                        sample_frame_buf[get_index(x, y)][i] = fragment_shader(payload);
                    }
                }
            }
}
