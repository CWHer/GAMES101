...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    float u = scale * dotProduct(S1, S);
    float v = scale * dotProduct(S2, ray.direction);

    const double eps = 1e-2;
    if (t < eps || u < 0 || v < 0 || u + v > 1)
        return inter;

    auto dist_vec = t * ray.direction;
    inter.happened = true;
    inter.coords = ray.origin + dist_vec;
    inter.normal = normal;
    inter.distance = dotProduct(dist_vec, dist_vec);
    inter.obj = this;
    inter.m = m;
